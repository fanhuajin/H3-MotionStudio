...
- 去字幕遮罩位置自动检测：固定"贴底"遮罩对竖屏歌词字幕（多在画面 0.75–0.85H、非贴底）会落空，因此提交去字幕工作流前由 `backend/subtitle_detect.py` 自动定位持续字幕条——只取视频 30%~97% 时段抽帧（≤1280 宽等宽缩放、跳过片头标题），逐帧算行边缘能量的「相对 ±110px 邻域尖峰比 ≥1.45」得候选带，跨帧投票（≥40% 采样帧出现、带宽 10–260px），带内列覆盖 p2~p98 定横向范围并外扩边距；检测结果按 VHS 拉伸映射（画布坐标 = 源坐标 × 画布/源尺寸）换算后覆盖 `CANVAS_PARAMS` 的 mask 参数（`mask_params_from_box`），检测失败/关闭时回退固定底部。检测器用 ComfyUI 自带 python（`COMFY_PYTHON`，含 numpy/Pillow）执行；环境变量 `H3_SUBTITLE_DETECT=0` 关闭自动检测。
